...
list_link Create_Link_List_Node()//创建节点就是申请堆空间
{
    ...
    node_heap_start_p->next = node_heap_start_p;//提高可读性
...
int  Tail_Add_Link_List_Node(list_link head_node) {
    ...
    //遍历直接找尾结点
    list_link tmp_node;
    for (tmp_node = head_node;tmp_node->next !=head_node;tmp_node=tmp_node->next);
    {
...
list_link Search_Link_List_Node(list_link head_node,char * search_mode)//添加查找模式，当前或上一个
{
    ...
    else
    {
        ...
        //找上一个，从头的下一个开始找
        //找当前，从头的下一个开始找
        for(list_link obj_node = head_node;obj_node->next !=head_node;obj_node = obj_node->next)
        {
...
int Move_Link_List_Node(list_link head_node)
{
    ...
    else if(head_node->next == head_node|| head_node->next -> next== head_node)
    {
        printf("只有一个节点，无法移动！\n");
        return 0;
    }
